...
public class Demodulator extends Thread
{
   ...
   static final org.apache.logging.log4j.Logger logger = LogManager.getLogger(PropertiesWrapper.class.getName());
    ...
    public void run()
    {
        ...
        logger.info("Demodulator started, sample rate : " + sampleRate + ", audioDecimationRate : " + audioDecimationRate);
        ...
        while (!stopRequest)
        {
            try
            {
                ...
                IQAudioIn = (double[][]) lbdDemodulatorInput.poll(50, TimeUnit.MILLISECONDS);

                if (IQAudioIn != null)
                {
                    logger.debug("new IQAudio packet with size : " + IQAudioIn.length + ", queue : " + lbdDemodulatorInput.size());
                    ...
                    for (int i = 0; i < FFTSIZE; i++)
                    {
                        ...
                        PPFilterInI[subFilterSwitch] = IQAudioIn[i][0];
                        PPFilterInQ[subFilterSwitch] = IQAudioIn[i][1];
