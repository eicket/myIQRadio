...
public class ListAudioOutDevices
{
    ...
    public static void ListAudioOut(ChoiceBox audioOutBox)
    {
        ...
        for (int i = 0; i < aInfos.length; i++)
        {
            ...
            if (mixer.isLineSupported(new Line.Info(SourceDataLine.class)))
            {
                ...
                if ((mixer.getMixerInfo().getName()).equalsIgnoreCase(propWrapper.getStringProperty("AudioOut")))
                {
                    logger.debug("Found received AudioOut device in property file : " + mixer.getMixerInfo().getName());
