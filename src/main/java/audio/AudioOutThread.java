...
public class AudioOutThread extends Thread
{
    ...
    static final org.apache.logging.log4j.Logger logger = LogManager.getLogger(ListAudioOutDevices.class.getName());
    ...
    {
        logger.debug("Audio out thread is started");
        ...
        for (int i = 0; i < aInfos.length; i++)
        {
            ...
            logger.debug("next mixer : " + mixer.getMixerInfo().toString());

            // test for a target == audio in
            if (mixer.isLineSupported(new Line.Info(SourceDataLine.class)))
            {
                logger.debug("audio out - target : " + mixer.getMixerInfo().getName());

                if ((mixer.getMixerInfo().getName()).equalsIgnoreCase(device))
                {
                    logger.debug("Found received AudioOut device : " + device);
                    ...
                    {
                        logger.fatal("Can't get SourceDataLine : " + ex.getMessage());
                    }
        ...
        {
            logger.fatal("LineUnavailableException : " + ex.getMessage());
        }
        ...
        {
            logger.fatal("sourceDataLine is not open");
        }
    }
    ...
    public void run()
    {
        ...
        while (!stopRequest)
        {
            try
            {
                logger.debug("Audio out free buffer : " + (sourceDataLine.getBufferSize() - sourceDataLine.available()));
                if (sourceDataLine.available() == sourceDataLine.getBufferSize())
                {
                    logger.debug("Audio out underrun");
                }
                ...
                if (audioOutFromQueue != null)
                {
                    ...
                    logger.debug("Write : " + len + ", dump : " + Arrays.toString(audioOutFromQueue));
                    logger.debug("New audio out buffer : " + audioOutFromQueue.length + ", available for write : " + sourceDataLine.available() + ", internal buffer size : " + sourceDataLine.getBufferSize());
                    ...
                    for (int i = 0; i < audioOutFromQueue.length; i++)
                    {
                        ...
                        logger.debug("Audio out, out : " + out + ", hi : " + hi + ", lo : " + lo);
                    }

                    int nAudioOut = sourceDataLine.write(audioOutToSoundCard, 0, audioOutToSoundCard.length);
                    logger.debug("Number of bytes written : " + nAudioOut + ", available for write : " + sourceDataLine.available());
                }
            }
